...
import { createWhatsAppOrderURL } from "../../util/whatsapp";
...
export function MenuCard({ item }: MenuCardProps) {
  const handleOrder = () => {
    const url = createWhatsAppOrderURL(item.name, item.price, item.plateSize);
    ...
    <div className="relative h-90 rounded-xl overflow-hidden group cursor-pointer">
      ...
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10">
        <img
          src={item.image}
          alt={item.name}
          className="w-100 h-60 object-contain drop-shadow-xl transition-transform duration-900 group-hover:scale-130"
        />
      </div>
      ...
      <div className="absolute bottom-4 left-4 right-4 z-10">
        <div className="flex justify-between items-center mb-2">
          ...
          <span className=" text-[#F5F5F5] font-poppins">{item.plateSize}</span>
        </div>
